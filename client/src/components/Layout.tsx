          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/dashboard" className="flex items-center space-x-3 group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-gray-400 dark:text-slate-500 font-semibold uppercase tracking-wider -mt-1">
                  Personalized Travel
                </span>
              </div>
            </Link>
              ...
              <nav className="hidden md:flex items-center space-x-1">
                {navLinks.map((link) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-4 h-4 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-slate-400'}`} />
                      <span>{link.name}</span>
                    </Link>
                  );
                })}
            ...
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <button
                onClick={toggleTheme}
                className="p-2 rounded-xl text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-800 transition"
              ...
              {user ? (
                <>
                  <Link
                    to="/create-trip"
                    className="hidden sm:flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-600/20 transition-all hover:scale-[1.02]"
                  >
          ...
          <div className="md:hidden border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 pt-2 pb-4 space-y-2">
            ...
            <div className="pt-2 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <Link
                to="/create-trip"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center space-x-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold"
              >
                <PlusCircle className="w-4 h-4" />
                <span>Plan New Trip</span>
              </Link>
