...
export const TripCard: React.FC<TripCardProps> = ({ trip, onDelete }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#111E2E] dark:hover:bg-[#162235] rounded-2xl shadow-sm hover:shadow-xl border border-slate-200 dark:border-[#1E2D42] overflow-hidden transition-all duration-300 group flex flex-col justify-between">
        ...
        <div className="relative h-48 w-full overflow-hidden bg-slate-100 dark:bg-[#162235]">
          ...
          {onDelete && (
            <button
              ...
              className="absolute top-3 right-3 p-2 bg-white/80 dark:bg-[#0B1320]/80 hover:bg-rose-50 dark:hover:bg-rose-950/60 hover:text-rose-600 rounded-full backdrop-blur-md text-slate-600 dark:text-slate-300 transition"
              title="Delete Trip"
            >
          ...
          <div className="mt-4 space-y-2 text-xs text-slate-600 dark:text-slate-300">
            ...
            {trip.totalBudget > 0 && (
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200">${trip.totalBudget.toLocaleString()} Budget</span>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Footer link actions */}
      <div className="px-5 py-3.5 bg-slate-50 dark:bg-[#162235]/60 border-t border-slate-100 dark:border-[#1E2D42] flex items-center justify-between">
        <Link
          to={`/trips/${trip.id}`}
