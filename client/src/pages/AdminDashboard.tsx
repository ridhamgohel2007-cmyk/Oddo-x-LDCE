  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-16">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 flex items-center space-x-3">
            ...
            <span>Admin Panel / Analytics Dashboard</span>
          </h1>
          <p className="text-xs text-gray-500 mt-1">Screen 12: Admin-only interface to track user trends, trip adoption, and platform usage</p>
        </div>
        ...
        <div className="flex bg-gray-100 p-1.5 rounded-2xl">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition ${
              activeTab === 'ANALYTICS' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition ${
              activeTab === 'USERS' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600'
            }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-bold text-gray-400 uppercase">Total Trips Created</span>
          <div className="text-3xl font-black text-emerald-600">{summary.totalTrips}</div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-bold text-gray-400 uppercase">Destinations</span>
          <div className="text-3xl font-black text-blue-600">{summary.totalCities}</div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-bold text-gray-400 uppercase">Community Shares</span>
          <div className="text-3xl font-black text-purple-600">{summary.totalCommunityPosts}</div>
        </div>
      </div>
      ...
      {activeTab === 'ANALYTICS' ? (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-3">
              <h3 className="text-base font-bold text-gray-900">Popular Cities Ranking</h3>
              <div className="space-y-2">
                {popularCities.map((c: any, i: number) => (
                  <div key={c.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-2xl text-xs font-semibold">
                    <span className="text-gray-800">#{i + 1} {c.name}, {c.country}</span>
                    <span className="text-emerald-700 font-extrabold">{c.popularityScore} Score</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-3">
              <h3 className="text-base font-bold text-gray-900">Popular Activities Ranking</h3>
              <div className="space-y-2">
                {popularActivities.map((a: any, i: number) => (
                  <div key={a.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-2xl text-xs font-semibold">
                    <span className="text-gray-800">#{i + 1} {a.title} ({a.city?.name})</span>
                    <span className="text-blue-700 font-extrabold">${a.estimatedCost}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 space-y-4">
          <h2 className="text-lg font-extrabold text-gray-900">Manage Registered Users</h2>
              ...
              <tbody className="divide-y divide-gray-100">
                {users.map((u: any) => (
                  <tr key={u.id} className="hover:bg-gray-50/80">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${u.role === 'ADMIN' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-700'}`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="p-3 font-bold text-emerald-700">{u._count?.trips || 0}</td>
